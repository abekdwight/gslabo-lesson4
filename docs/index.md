...
今日扱う1つ1つの要素は、最初のうちはピンとこないかもしれません。いずれも、Laravel のコードの品質を保つために頻繁に使うことになる、重要で基本的な機能です。今日の時点で完璧に覚える必要はありません。「こういうときは、こうすればいい」と頭の片隅に置いておいて、LLM にコードを書かせるときの指示に使ってください。モデルの性能にもよりますが、LLM は何も指示しなければ、こうした機能を使わないことがあります。
