...
6. 表示の変換をモデルに移す（アクセサ）
7. `with('category')` は何をしているか

①が仕上げ、②〜④がフォームまわり、⑤〜⑦がモデルまわりです。

## ① 編集と削除を作る

編集と削除で書くのは、ルート・コントローラ・ビュー・バリデーションで、前回の登録・一覧と同じです。新しく出てくる仕組みは、ルートモデルバインディングと `@method()` の2つだけです。コードを貼って動かしながら、その2つだけ説明します。
