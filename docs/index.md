...
2. 家計簿のコードを、**Laravel が用意している仕組みを使う形にする**。保存したら結果を伝える、検証ルールを1箇所にまとめる、エラーメッセージを日本語にする

増える画面は編集画面の1つだけで、あとは動いている家計簿に Laravel の機能を1つずつ足していきます。

今日扱う1つ1つの要素は、最初のうちはピンとこないかもしれません。

いずれも、Laravel のコードの品質を保つために頻繁に使うことになる、重要で基本的な機能です。
今日の時点で完璧に覚える必要はありません。
「こういうときは、こうすればいい」と頭の片隅に置いておいて、LLM にコードを書かせるときの指示に使ってください。

モデルの性能にもよりますが、LLM は何も指示しなければ、こうした機能を使わないことがあります。
...
①で家計簿の操作が一通りそろいます。②〜④はフォームまわりです。

## ① 編集と削除を作る

編集と削除で書くのは、ルート・コントローラ・ビュー・バリデーションで、前回の登録・一覧と同じです。

新しく出てくる仕組みは、ルートモデルバインディングと `@method()` の2つです。

ルートは前回 `Route::resource('transactions', ...)->except(['show'])` と宣言した時点で6本そろっているので、追記はありません。

編集と削除が使う URL の形を、`route:list` の結果で確認します。
...
!!! warning "注意：開発環境だけで使う"

    Debugbar は、収集したリクエストの情報を画面に表示するツールです。本番など外部から見える環境で有効にすると、その情報が誰にでも見えてしまいます。開発環境だけで使ってください。また、情報を集めて描画するぶん表示は遅くなります。データの重いページで遅さが気になるときは、`config/debugbar.php` で使わないコレクターを無効にします。
...
!!! note "補足：丸ごとコピーする理由"
    ...
    丸ごとコピーしてから置き換えていくと、このファイルがそのまま対応表になります。英語のままの行はまだ訳していない部分、日本語になった行は置き換え済みの部分だと、開けばいつでも分かります。必要な行だけを書き足していく作り方だと、何が対応できていて何が残っているかが分かりません。
...
ここから先は、時間内に終わらなくても構いません。上から順に進めて、途中で終わっても構いません。

### 日付をオブジェクトとして扱う（$casts）

一覧の日付は `2026-08-01` と表示されています。これを「8月1日」の形式にすることを考えます。まず、いまの `occurred_at` に何が入っているかを tinker で確認します。
...
一覧のビューには、1つ前の節で整えた日付のほかにも、表示のための変換が2つ書いてあります。区分の `{{ $transaction->type === 'income' ? '収入' : '支出' }}` と、金額の `¥{{ number_format($transaction->amount) }}` です。動いてはいますが、この書き方では表示の変換がビューに増えていきます。この変換はモデル側に持たせられます。その仕組みが**アクセサ**です。
